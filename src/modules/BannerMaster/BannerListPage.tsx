...
import { PlusCircle, Edit, Trash2, Search, ChevronsUpDown, ArrowDown, ArrowUp, Image as ImageIcon } from 'lucide-react';
...
const BACKEND_URL = process.env.NODE_ENV === 'production' ? '' : 'http://localhost:3000'; // Adjust as needed for production
...
const BannerMasterListPage: React.FC = () => {
  ...
  const fetchBanners = useCallback(async () => {
    ...
    try {
      ...
      const responseData = await apiService.get<{ banners: Banner[], totalPages: number, totalRecords: number }>(API_BASE_URL, params);
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="px-6 py-4 border-b">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <CardTitle className="text-2xl font-semibold">Banner Management</CardTitle>
              ...
              <div className="overflow-x-auto">
                <Table className="min-w-full">
                  <TableHeader className="bg-slate-50 dark:bg-slate-800">
                    <TableRow>
                      <TableHead onClick={() => handleSort('listOrder')} className="cursor-pointer px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center">Order <SortIndicator column='listOrder' /></div>
                      </TableHead>
                      <TableHead className="px-4 py-3 whitespace-nowrap">Image</TableHead>
                      <TableHead onClick={() => handleSort('caption')} className="cursor-pointer px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center">Caption <SortIndicator column='caption' /></div>
                      </TableHead>
                      <TableHead className="px-4 py-3 whitespace-nowrap">Description</TableHead>
                  ...
                  <TableBody>
                    {banners.map((banner) => (
                      <TableRow key={banner.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                        <TableCell className="px-4 py-3 font-medium">{banner.listOrder}</TableCell>
                        <TableCell className="px-4 py-3">
                          {banner.imagePath ? (
                            <img src={`${BACKEND_URL}${banner.imagePath}`} alt={banner.caption || 'Banner image'} className="h-10 w-16 object-cover rounded" />
                          ) : (
                            <span className="text-xs text-muted-foreground">No image</span>
                          )}
                        </TableCell>
                        <TableCell className="px-4 py-3">{banner.caption || '-'}</TableCell>
                        <TableCell className="px-4 py-3 truncate max-w-xs">{banner.description || '-'}</TableCell>
                        <TableCell className="px-4 py-3">{new Date(banner.createdAt).toLocaleDateString()}</TableCell>
                        <TableCell className="text-right px-4 py-3">
                          <Button variant="ghost" size="icon" onClick={() => handleEdit(banner)} className="hover:text-blue-600">
                            <Edit className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => handleDeleteConfirmation(banner)} className="hover:text-red-600">
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the banner "<strong>{bannerToDelete.caption || 'this banner'}</strong>".
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
