...
interface OrderItem {
  ...
  receivedQuantity?: number; // Received quantity from backend
  supervisorQuantity?: number; // Supervisor quantity from backend
  agencyId?: string;
  agencyName?: string;
  depotId?: string;
  depotName?: string;
...
const SupervisorQtyPage = () => {
  ...
  const recordSupervisorQtyMutation = useMutation({
    mutationFn: (supervisorData: { items: { orderItemId: string; supervisorQuantity: number }[] }) => {
      if (!orderId) throw new Error("Order ID is missing for supervisor quantity submission.");
      return put(`/vendor-orders/${orderId}/record-supervisor-quantity`, supervisorData);
    },
    onSuccess: () => {
      toast.success("Supervisor quantity recorded successfully!");
      queryClient.invalidateQueries({ queryKey: ["order", orderId] }); 
      queryClient.invalidateQueries({ queryKey: ["orders"] }); 
      navigate(`/admin/orders`); 
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || "Failed to record supervisor quantity. Please try again.");
      console.error("Submit supervisor quantity error:", error);
    },
  });

  const { data: order, isLoading, isError } = useQuery<Order>({
    queryKey: ["order", orderId, "supervisor-quantity"],
  ...
  const handleSubmitSupervisorQty = () => {
    if (!order || !order.items || !currentUserProfile) { 
      toast.error("Order data or user profile is not loaded yet.");
      return;
    }

    // For supervisors, consider all items for submission
    const itemsToSubmit = Object.entries(supervisorQuantities)
      .map(([itemId, quantityStr]) => {
        const itemDetail = order.items.find(i => i.id === itemId);
        if (!itemDetail) return null; 

        const supervisorQuantity = parseInt(quantityStr, 10);
        if (isNaN(supervisorQuantity) || supervisorQuantity < 0) {
        ...
        <div className="lg:w-2/3 space-y-4 md:space-y-6">
          <Card>
            <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 space-y-2 sm:space-y-0">
              <CardTitle className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800 dark:text-gray-100 leading-tight">Record Supervisor Quantity for Order #{order.poNumber}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 md:space-y-6">
              ...
              <div className="space-y-2 pt-4">
                ...
                <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Enter the supervisor quantity for each item. This cannot exceed the received quantity.</p>
              </div>
                ...
                <table className="w-full text-sm min-w-[700px]">
                  <thead className="bg-gray-100 dark:bg-gray-800">
                    <tr>
                      ...
                      <th className="px-2 sm:px-4 py-2 text-right font-medium text-gray-600 dark:text-gray-300 text-xs sm:text-sm min-w-[80px]">Received</th>
                      <th className="px-2 sm:px-4 py-2 text-right font-medium text-gray-600 dark:text-gray-300 text-xs sm:text-sm min-w-[120px]">Supervisor Qty</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {(order?.items || []).map(item => (
                      <tr key={item.id}>
                        ...
                        <td className="px-2 sm:px-4 py-3 text-right text-gray-600 dark:text-gray-300 text-xs sm:text-sm">
                          <div className="font-medium">{item.receivedQuantity != null ? item.receivedQuantity : 'N/A'}</div>
                          {item.receivedQuantity != null && item.depotVariantName && <div className="text-xs text-gray-500 truncate">({item.depotVariantName})</div>}
                        </td>
...
export default SupervisorQtyPage;