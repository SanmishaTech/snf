import { useEffect } from "react"; // Re-added useEffect
...
const UserForm = ({ mode, userId, onSuccess, className }: UserFormProps) => {
  ...
  const createUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["users"] }); // Refetch the users list
                  ...
                  <SelectContent>
                    {roles.map((roleValue) => (
                      <SelectItem key={roleValue} value={roleValue}>
                        {formatRoleName(roleValue)} {/* Display formatted role name */}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
