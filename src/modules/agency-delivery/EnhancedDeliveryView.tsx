...
const EnhancedDeliveryView: React.FC = () => {
  ...
  // useEffect to fetch expiring subscriptions for AGENCY
  useEffect(() => {
    const fetchExpiring = async () => {
      if (currentUser?.role !== 'AGENCY') return;
      
      setLoadingExpiring(true);
      try {
        const subs = await get<any[]>('/subscriptions');
        const now = new Date();
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 bg-gray-100 min-h-screen font-sans">
      ...
      {currentUser?.role === 'AGENCY' && (expiringSubscriptions.length > 0 || loadingExpiring) && (
        <section className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              <span className="flex h-3 w-3 rounded-full bg-orange-500 animate-pulse"></span>
              Subscription Alerts (Expired/Expiring Soon)
            </h2>
            <span className="text-xs font-medium px-2.5 py-0.5 rounded-full bg-orange-100 text-orange-800">
              {expiringSubscriptions.length} Actions Required
            </span>
          </div>

          {loadingExpiring ? (
            <div className="flex justify-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
              <Spinner size="sm" />
            </div>
          ) : (
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h3 className="font-bold text-gray-900 leading-tight">
                            {sub.member?.user?.name || 'Unknown Customer'}
                          </h3>
                          <a 
                            href={`tel:${sub.member?.user?.mobile}`} 
                            className="text-xs font-medium text-indigo-600 hover:underline flex items-center gap-1 mt-1"
                          >
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            {sub.member?.user?.mobile || 'No Mobile'}
                          </a>
                        </div>
                        <span className={clsx(
                          "text-[10px] uppercase font-bold px-2 py-1 rounded",
                          isExpired ? "bg-red-600 text-white" : "bg-orange-600 text-white"
                        )}>
                          {isExpired ? 'Expired' : 'Nearing'}
                        </span>
                      </div>
                      ...
                      <div className="mt-2 space-y-1">
                        <div className="flex items-center justify-between text-xs text-gray-600">
                          <span>Product:</span>
                          <span className="font-semibold text-gray-800">{sub.product?.name}</span>
                        </div>
                        <div className="flex items-center justify-between text-xs text-gray-600">
                          <span>Expiry:</span>
                          <span className={clsx("font-semibold", isExpired ? "text-red-700" : "text-orange-700")}>
                      ...
                      <div className={clsx(
                        "mt-4 pt-3 border-t text-sm font-bold flex items-center gap-2",
                        isExpired ? "border-red-200 text-red-700" : "border-orange-200 text-orange-700"
                      )}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        {isExpired ? 'Already ended' : `Ends in ${diffDays} day${diffDays !== 1 ? 's' : ''}`}
                      </div>
                    </div>
                  </div>
                );
